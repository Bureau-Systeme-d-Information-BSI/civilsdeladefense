...
export default class extends Controller {
  removeRecord(event) {
    ...
    let hidden_field = job_offer_actor_node.querySelector(
      '[name$="[_destroy]"]',
    );
  ...
  addFields(event) {
    document.querySelector("#admin-not-found")?.classList?.add("d-none");

    let node = event.currentTarget;
    let emailField =
      node.previousElementSibling.querySelector("input[name=email]");
    if (emailField == null) {
    ...
    Rails.ajax({
      ...
      beforeSend: function () {
        ...
        [].forEach.call(
          formGroup.querySelectorAll(".invalid-feedback"),
          function (el) {
